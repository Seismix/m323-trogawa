...
for p in products:
    if p.price > 50:
        result.append(Product(p.name, p.price * 0.9))
    else:
        result.append(p)

# Deklarativ (Ergebnis beschreiben)
result = [
    Product(p.name, p.price * 0.9) if p.price > 50 else p
    for p in products
]
```

Beide Varianten liefern dasselbe Ergebnis. Die deklarative Variante spiegelt die Anforderung fast wörtlich wider: "Produktliste mit reduzierten Preisen für Produkte über 50."
