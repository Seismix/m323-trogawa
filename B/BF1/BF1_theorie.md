...
```java
record User(String email, LocalDate lastLogin) {}

var cutoff = LocalDate.now().minusDays(90);

// Imperative Umsetzung (Schritte)
var result = new ArrayList<String>();
for (var user : users) {
    if (user.lastLogin().isBefore(cutoff)) {
...
var result = users.stream()
    .filter(user -> user.lastLogin().isBefore(cutoff))
    .map(User::email)
    .toList();
```
...
Wichtig: Die deklarative Anforderung ist **nicht** die Implementierung. Sie beschreibt nur das gewünschte Ergebnis. Ob die Implementierung mit der Stream API, einer `for`-Schleife oder einer SQL-Abfrage erfolgt, ist eine separate Entscheidung.
